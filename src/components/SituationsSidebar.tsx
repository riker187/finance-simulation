  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-4 border-b border-slate-800">
        <h2 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">{t('Situationen')}</h2>
        <p className="text-xs text-slate-600 mt-0.5">{t('Finanzielle Bausteine')}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-2 space-y-3 px-2">
        {situations.length === 0 && (
          <div className="text-center py-8 text-slate-600 text-sm">
            {t('Noch keine Situationen.')}<br />{t('Erstelle deine erste!')}
          </div>
        )}
              ...
              >
                    ...
                    <div className="text-xs text-slate-500 mt-1 space-y-0.5">
                      {sit.effects.map((e) => (
                        <div key={e.id} className="flex items-center gap-1">
                          ...
                          >
                            {e.category === 'income' ? '+' : '-'}
                            {e.amount.toLocaleString(lang === 'en' ? 'en-US' : 'de-DE')} EUR
                          </span>
                          <span className="text-slate-600">
                            {e.type === 'one-time' ? t('(einmalig)') : t('/Mo.')}
                          </span>
                        </div>
                      ))}
                      {sit.effects.length === 0 && (
                        <span className="text-slate-600 italic">{t('Keine Auswirkungen')}</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                {confirmDelete === sit.id && (
                  <div className="mt-2 flex gap-2 items-center text-xs">
                    <span className="text-slate-400">{t('Wirklich löschen?')}</span>
                    <button
